    ...
    <div class="flex justify-between items-center mb-3">
        <div>
            ...
            <div class="flex items-center mt-1">
                ...
                // Get memo number from the transaction
                $memoNumber = $trx->memo_number ?? 1;
                @endphp
                <span class="w-7 h-7 flex items-center justify-center rounded-full bg-blue-100 text-blue-800 font-medium mr-2 text-sm">
                    {{ $initials }}
                </span>
                <div>
                    <div class="text-sm font-medium text-gray-600">{{ $initials }}-{{ $memoNumber }}</div>
...
{{-- Desktop Table Layout Rows --}}
@foreach($transactions as $trx)
<tr class="hover:bg-gray-50 transition duration-150 ease-in-out desktop-row">
    <td class="px-6 py-4 whitespace-nowrap">
        <span class="text-sm font-medium text-gray-900">{{ $trx->ledger->name }}</span>
    </td>
    <td class="px-6 py-4 whitespace-nowrap">
        <span class="text-sm text-gray-500">
            ...
            <div class="flex items-center">
                <span class="w-8 h-8 flex items-center justify-center rounded-full bg-blue-100 text-blue-800 font-medium mr-2">
                    {{ $initials }}
                </span>
                <div>
                    <div class="text-xs text-gray-500">{{ $initials }}-{{ $memoNumber }}</div>
                    ...
                    <div class="text-xs text-gray-400 mt-1">
                        ...
                        @endphp
                        <span class="inline-flex items-center justify-center bg-gray-100 text-gray-600 rounded-full px-2 py-0.5 text-xs mr-1">
                            {{ $contInitials }}
                        </span>
                        @endforeach
                    </div>
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        <span class="text-sm font-semibold text-gray-900">{{ number_format($trx->grand_total) }}</span>
    </td>
    <td class="px-6 py-4 whitespace-nowrap text-center">
        <div class="flex justify-center space-x-3">
            <!-- Product Return Icon (Desktop) -->
            <a href="javascript:void(0)"
                class="return-btn {{ $trx->grand_total <= 0 ? 'text-gray-400 cursor-not-allowed' : 'text-amber-500 hover:text-amber-700 transition duration-300' }}"
                data-id="{{ $trx->id }}"
                {{ $trx->grand_total <= 0 ? 'disabled' : '' }}
                title="{{ $trx->grand_total <= 0 ? 'Returns not available for fully returned transactions' : 'Return Products' }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 100 100" enable-background="new 0 0 100 100" xml:space="preserve" fill="currentColor">
                    <g>
                        <path d="M25.8,60.6h-4.5c-0.7,0-1.3,0.6-1.3,1.3v16.1c0,0.7,0.6,1.3,1.3,1.3h2.2c2,0,3.6-1.6,3.6-3.6V62C27.2,61.2,26.5,60.6,25.8,60.6z" />
                        <path d="M79.9,69.4c-0.7-1.6-2-3.3-3.9-3.5c-1-0.1-2,0.3-2.9,0.6c-3.6,1.3-7.2,2.5-10.8,3.8c-2.3,0.8-4.7,1.6-7.2,1.8c-1.7,0.1-3.4,0-5.1,0c-0.9,0-1.7-0.7-1.7-1.7s0.7-1.7,1.7-1.7l9.1,0c1.7,0,3-1.4,3-3s-1.4-3-3-3h-7c-0.3,0-2.2-0.1-3.4-0.6c-1.3-0.6-3-0.7-3-0.6c0,0,0,0-0.1,0H33.4c-1.5,0-2.7,1.2-2.7,2.7v11.3c0,1.3,1,2.4,2.3,2.6c0.1,0,0.2,0,0.3,0c2.3,0,4.6,0.5,6.9,0.9c2.3,0.5,4.5,0.8,6.9,0.8c3,0.1,6.1-0.4,9-1.1c2.9-0.8,5.7-1.9,8.5-2.8c4.8-1.6,9.7-3.3,14.5-4.9C79.7,70.7,80.2,70.2,79.9,69.4z" />
                        <path fill-rule="evenodd" clip-rule="evenodd" d="M58,40.1v15c0,0.5,0.5,0.7,0.9,0.6c2.9-1.7,11.9-6.7,11.9-6.7c1.2-0.7,1.9-1.9,1.9-3.3V32.2c0-0.5-0.5-0.7-0.9-0.6l-13.2,7.4C58.3,39.3,58,39.7,58,40.1" />
            ...
            <!-- View Icon -->
            <a href="{{ route('admin.inventory.inventory_transactions.show', $trx->id) }}" class="text-blue-500 hover:text-blue-700 transition duration-300" title="View Transaction">
            ...
            <a href="javascript:void(0)"
                class="collection-btn {{ ($trx->payment_method === 'credit' && $trx->grand_total > 0) ? 'text-green-500 hover:text-green-700' : 'text-gray-400 cursor-not-allowed' }} transition duration-300"
                data-id="{{ $trx->id }}"
                data-amount="{{ $trx->grand_total }}"
                {{ ($trx->payment_method !== 'credit' || $trx->grand_total <= 0) ? 'disabled' : '' }}
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 {{ ($trx->payment_method !== 'credit' || $trx->grand_total <= 0) ? 'opacity-50' : '' }}" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M4 4a2 2 0 00-2 2v1h16V6a2 2 0 00-2-2H4z" />
                    <path fill-rule="evenodd" d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" clip-rule="evenodd" />
                </svg>
            </a>
            ...
            <!-- Share Icon -->
            <button class="text-indigo-500 hover:text-indigo-700 transition duration-300" title="Share Transaction">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M15 8a3 3 0 10-2.977-2.63l-4.94 2.47a3 3 0 100 4.319l4.94 2.47a3 3 0 10.895-1.789l-4.94-2.47a3.027 3.027 0 000-.74l4.94-2.47C13.456 7.68 14.19 8 15 8z" />
                </svg>
            </button>
